...
**結論**: メタモードによりツール選択成功率が 29% → 95%+ に向上。新メタモード（v2.0）では
メタツール定義を 31% スリム化し、同等の成功率を維持。`get_tool_schema` の廃止、
`list_upstream_tools` の追加、全 docstring の短縮により、コンテキスト効率と
LLMの発見容易性を両立する。**デフォルトでON**。
