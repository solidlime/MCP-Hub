...
|-----------|--------|:-----:|:---:|:---:|
| stdio | filesystem | 13 | 12/12 (100%) | 12/12 (100%) |
| stdio | sequential-thinking | 1 | 3/3 (100%) | 1/3 (33%) |
| Streamable HTTP | exa (web_search, web_fetch) | 2 | ✗ timeout | ✗ timeout |
| SSE | sse-echo | 1 | 3/3 (100%) | 3/3 (100%) |
| Streamable HTTP (202) | async-mcp | 1 | 3/3 (100%) | 0/3 (0%) |
| **合計** | | **18** | **21/21 (100%)** | **16/21 (76%)** |

> **注**: sequential-thinking の Meta OFF 失敗は stdio プロキシのセッション競合が原因。
> async-mcp の Meta OFF 失敗は 202 ポーリング対応が Meta ON 経由でのみ有効なため。
> **Meta ON は全プロトコルで 100% 成功**。

### ツール定義サイズ比較

| モード | メタツール数 | tools/list サイズ | 成功率 |
|--------|:---------:|:-----------------:|:------:|
| 通常モード (N=18) | — | 18ツール分の全スキーマ | 76% |
| メタモード | 3 | 3ツールのみ | **100%** |

**結論**: メタモードによりツール定義サイズを大幅削減しつつ、ツール呼び出し成功率 100% を達成。
全3プロトコル（stdio/SSE/Streamable HTTP）で実証。**デフォルトでON**。
